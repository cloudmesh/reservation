...
class ReservationClient(object):
    ...
    def add(self, rsv):
        ...
        if(check_overlap(self.service, rsv) == True):
            ...
            completedRsv = self.service.events().insert(
                calendarId='primary', body=rsv).execute()
    ...
    def get_all(self):
        ...
        while True:
            ...
            for event in events['items']:
                event_list.append(
                    self.service.events().get(calendarId='primary', eventId=event['id']).execute())
            page_token = events.get('nextPageToken')
            if not page_token:
                break
        return event_list
    ...
    def reschedule(self, oldRsvId, newRsv):
        ...
        self.service.events().update(
            calendarId='primary', eventId=oldRsvId, body=newRsv).execute()
    ...
    def duration(self, rsvId):
        ...
        rsvEvent = self.service.events().get(
            calendarId='primary', eventId=rsvId).execute()
        duration_time = {'start_time:': rsvEvent['start'][
            'dateTime'], 'end_time:': rsvEvent['end']['dateTime']}
